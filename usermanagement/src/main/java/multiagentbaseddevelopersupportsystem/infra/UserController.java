...
public class UserController {
    ...
    @RequestMapping(
        value = "/users/{id}/acceptprojectinvitation",
        method = RequestMethod.PUT,
        produces = "application/json;charset=UTF-8"
    )
    public User acceptProjectInvitation(
        @PathVariable(value = "id") Long id,
        @RequestBody AcceptProjectInvitationCommand acceptProjectInvitationCommand,
        HttpServletRequest request,
        HttpServletResponse response
    ) throws Exception {
        System.out.println("##### /user/acceptProjectInvitation  called #####");
        Optional<User> optionalUser = userRepository.findById(id);

        optionalUser.orElseThrow(() -> new Exception("No Entity Found"));
        User user = optionalUser.get();
        user.acceptProjectInvitation(acceptProjectInvitationCommand);
