    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="loginDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('user')">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="acceptProjectInvitationDialog = true" class="contrast-primary-text" small color="primary" >
                    <v-icon small>mdi-minus-circle-outline</v-icon>초대요청수락
                </v-btn>
                <v-dialog v-model="acceptProjectInvitationDialog" width="500">
                    <AcceptProjectInvitation
                        @closeDialog="acceptProjectInvitationDialog = false"
...
export default {
    ...
    data: () => ({
        ...
        signupDialog: false,
        loginDialog: false,
        acceptProjectInvitationDialog: false,
    }),
    watch: {
    },
    methods:{
        ...
        async acceptProjectInvitation(params){
            try{
                var path = "acceptProjectInvitation".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','AcceptProjectInvitation 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.acceptProjectInvitationDialog = false
            }catch(e){
                console.log(e)
            }
        },
