    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="savePostDialog = true" class="contrast-primary-text" small color="primary" >
                    <v-icon small>mdi-minus-circle-outline</v-icon>게시글 저장
                </v-btn>
                <v-dialog v-model="savePostDialog" width="500">
                    <SavePost
                        @closeDialog="savePostDialog = false"
                        @savePost="savePost"
                    ></SavePost>
                </v-dialog>
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="deletePost" class="contrast-primary-text" small color="primary" >
                    <v-icon small>mdi-minus-circle-outline</v-icon>게시글 삭제
                </v-btn>
                <v-btn style="margin-left: 5px;" @click="startPostWriting" class="contrast-primary-text" small color="primary" >
                    <v-icon small>mdi-minus-circle-outline</v-icon>게시글 작성 시작
                </v-btn>
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="increaseViewCount" class="contrast-primary-text" small color="primary" >
...
export default {
    ...
    methods:{
        async savePost(params){
            try{
                var path = "savePost".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','savePost 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.savePostDialog = false
            }catch(e){
                console.log(e)
            }
        },
        ...
        async startPostWriting(){
            try{
                var path = "startPostWriting".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','startPostWriting 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
