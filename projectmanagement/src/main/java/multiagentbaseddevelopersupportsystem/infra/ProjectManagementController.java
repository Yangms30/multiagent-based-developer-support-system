...
public class ProjectManagementController {
    ...
    @RequestMapping(
        value = "/projectManagements/assignteamrole",
        method = RequestMethod.POST,
        produces = "application/json;charset=UTF-8"
    )
    public ProjectManagement assignTeamRole(
        HttpServletRequest request,
        HttpServletResponse response,
        @RequestBody AssignTeamRoleCommand assignTeamRoleCommand
    ) throws Exception {
        System.out.println(
            "##### /projectManagement/assignTeamRole  called #####"
        );
        ProjectManagement projectManagement = new ProjectManagement();
        projectManagement.assignTeamRole(assignTeamRoleCommand);
        projectManagementRepository.save(projectManagement);
        return projectManagement;
    }
}
